  ...
  return (
    ...
    border-b-[1px] hover:cursor-pointer hover:bg-orange-500'>
      ...
      <div>
        <h3>{user.name}</h3>
        {latestMessage && 
          <p className='overflow-hidden text-xs font-medium text-gray-600 break-words 
          whitespace-pre-wrap'>
            {latestMessage.text}
          </p>
        }
        {latestMessage && latestMessage.image &&
          <p className='text-xs font-medium text-gray-600 break-words whitespace-pre-wrap'>[이미지]</p>
        }
      </div>
